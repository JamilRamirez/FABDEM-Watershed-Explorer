...
# SUBMODULO: CAPACIDAD DE USO MAYOR
...
cum <- local({
  ...
  CUM_STEM <- "CUM_normalizada_disuelto"
  ...
  cum_index <- function() {
    ...
    if (
      ...
      if (!"RELATIVE_PATH" %in% names(idx)) {
        stop(
          "El index.gpkg de Capacidad de uso mayor no contiene RELATIVE_PATH."
        )
      }
  ...
  read_cum_for_basin <- function(basin) {
    ...
    if (!is.null(idx)) {
      ...
      if (nrow(idx_hit) == 0L) {
        stop(
          "Ninguna tesela de Capacidad de uso mayor intersecta la cuenca activa."
        )
      }
      ...
      if (length(missing_tiles) > 0L) {
        stop(
          paste0(
            "Falta una tesela de Capacidad de uso mayor:\n",
            missing_tiles[1]
          )
        )
      }
      ...
      original <- cum_original_file()

      if (!file_nonempty_local(original)) {
        stop(
          paste0(
            "No se encontró Capacidad de uso mayor teselada ni el GPKG original.\n",
            "Esperado:\n",
            cum_tiled_dir(),
            "\nó\n",
            original
      ...
      if (length(layers) < 1L) {
        stop("El GPKG de Capacidad de uso mayor no contiene capas vectoriales.")
      }
      ...
      if (!"CODIGO" %in% names(g)) {
        stop("El GPKG de Capacidad de uso mayor no contiene CODIGO.")
      }
    ...
    if (is.na(crs_ref)) {
      stop("La capa de Capacidad de uso mayor no tiene CRS válido.")
    }
    ...
    cum_all <- sf::st_make_valid(
      cum_all
    )

    cum_all <- cum_all[
      !sf::st_is_empty(cum_all),
      ,
      drop = FALSE
    ]
    ...
    if (nrow(cum_all) == 0L) {
      stop("Capacidad de uso mayor no contiene unidades dentro de la cuenca.")
    }
    ...
    if (nrow(clipped) == 0L) {
      stop("La intersección capacidad de uso mayor no produjo polígonos válidos.")
    }
  ...
  draw_cum_map <- function(x) {
    ...
    graphics::text(
      0.975,
      0.025,
      labels = layer_source_map_label(CUM_STEM),
      adj = c(1, 0),
      cex = 0.66,
      col = "grey30"
    )
  ...
  ui <- function(id) {
    ...
    shiny::tagList(
      shiny::tags$style(
        shiny::HTML(
          paste0(
            ".cum-wrap{padding:14px 18px 28px 18px;max-width:1500px;margin:auto;}",
            ".cum-head{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:12px;}",
            ".cum-note{background:#f6f7f8;border-left:4px solid #607D8B;padding:10px 12px;margin:10px 0 14px 0;}",
            ".cum-card{border:1px solid #ddd;border-radius:7px;padding:12px 12px 18px 12px;background:white;margin-bottom:16px;}",
            ".cum-card h4{margin-top:0;}",
            ".cum-a3-frame{width:1188px;max-width:100%;margin:0 auto;aspect-ratio:420/297;}",
            ".cum-table-wrap{width:100%;overflow-x:auto;padding-bottom:14px;}",
            ".cum-table-wrap .dataTables_wrapper{width:100%;}",
            ".cum-table-wrap table.dataTable{width:100%!important;}"
          )
        )
      ),
      ...
      shiny::div(
        ...
        shiny::div(
          class = "cum-note",
          "La capa de capacidad de uso mayor se carga únicamente para las geometrías que intersectan la cuenca activa. La leyenda muestra las clases presentes y la tabla inferior enlaza cada código con la metadata normalizada."
        ),

        layer_source_ui(CUM_STEM),
        ...
        shiny::conditionalPanel(
          ...
          shiny::div(
            class = "cum-card",
            shiny::tags$h4("Mapa de capacidad de uso mayor"),
            shiny::div(
              class = "cum-a3-frame",
              shiny::plotOutput(
                ns("mapa"),
          ...
          shiny::div(
            class = "cum-card",
            shiny::tags$h4("Clases de capacidad de uso mayor presentes"),
            shiny::div(
              class = "cum-table-wrap",
              DT::DTOutput(
                ns("tabla")
              )
            )
    ...
    shiny::moduleServer(
      ...
      function(
        ...
        status <- shiny::reactiveVal(
          "Delimita o carga una cuenca para consultar Capacidad de uso mayor."
        )
        ...
        shiny::observeEvent(
          current_key(),
          {
            result(NULL)
            status(
              "Cuenca disponible. Pulsa Cargar capacidad de uso mayor."
            )
          },
          ignoreInit = TRUE
        )
            ...
            tryCatch(
              {
                ...
                status(
                  "Localizando capa de capacidad de uso mayor..."
                )

                spatial <- read_cum_for_basin(
                  b
                )
              ...
              error = function(e) {
                result(NULL)
                msg <- paste0(
                  "Error en Capacidad de uso mayor: ",
                  conditionMessage(e)
                )
                status(msg)
        ...
        output$descargar_png <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            if (is.null(x)) {
              stop(
                "No hay un mapa de capacidad de uso mayor calculado para descargar."
              )
            }
        ...
        output$descargar_shp_recortado <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                cum_recortado = clipped
              ),
              target_file = file,
              bundle_stem = "cum_normalizado_recortado"
            )
          }
        )
        ...
        output$descargar_shp_mosaico <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              ...
              target_file = file,
              bundle_stem = "cum_normalizado_mosaico"
            )
          }
        )
