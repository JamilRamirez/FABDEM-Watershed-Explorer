...
# SUBMODULO: GEOLOGIA
# v4: fondo departamental limpio + mapa A3 + metadata + exportacion | arquitectura independiente del orden de carga
# ============================================================
...
geologia <- local({
  ...
  GEOLOGY_STEM <- "geologia_normalizada_disuelto"
  ...
  read_geology_metadata <- function() {
    if (exists(
      "geology",
  ...
  geology_original_file <- function() {
    file.path(
      LAYERS_DIR,
      "medio_fisico",
      paste0(
...
read_geology_for_basin <- function(basin) {
    ...
    if (!is.null(idx)) {
      ...
      if (nrow(idx_hit) == 0L) {
        stop(
          "Ninguna tesela de Geología intersecta la cuenca activa."
        )
      }
        ...
        function(tile_path) {
          tryCatch(
            ...
            error = function(e) {
              stop(
                paste0(
                  "No se pudo descargar una tesela de Geología.\n",
                  conditionMessage(e)
                )
              )
            }
      ...
      original <- geology_original_file()

      if (!file_nonempty_local(original)) {
        stop(
          paste0(
            "No se encontró Geología teselada ni el GPKG original.\n",
            "Esperado:\n",
            geology_tiled_dir(),
            "\nó\n",
            original
      ...
      if (length(layers) < 1L) {
        stop("El GPKG de Geología no contiene capas vectoriales.")
      }
      ...
      if (!"CODIGO" %in% names(g)) {
        stop("El GPKG de Geología no contiene CODIGO.")
      }
    ...
    if (is.na(crs_ref)) {
      stop("La capa de Geología no tiene CRS válido.")
    }
    ...
    geology_all <- sf::st_make_valid(
      geology_all
    )

    geology_all <- geology_all[
      !sf::st_is_empty(geology_all),
      ,
      drop = FALSE
    ]
    ...
    if (nrow(geology_all) == 0L) {
      stop("Geología no contiene unidades dentro de la cuenca.")
    }
    ...
    if (nrow(clipped) == 0L) {
      stop("La intersección geológica no produjo polígonos válidos.")
    }
  ...
  draw_geology_map <- function(x) {
    basin <- sf::st_transform(
      x$basin,
      4326
    )

    geology <- sf::st_transform(
      x$geology,
      4326
    )
    ...
    graphics::text(
      0.975,
      0.025,
      labels = layer_source_map_label(GEOLOGY_STEM),
      adj = c(1, 0),
      cex = 0.66,
      col = "grey30"
    )
  ...
  geology_datatable <- function(tab) {
    out <- tab
  ...
  ui <- function(id) {
    ...
    shiny::tagList(
      ...
      shiny::div(
        ...
        shiny::div(
          class = "geo-note",
          "La capa geológica se carga únicamente para las teselas que intersectan la cuenca activa. La leyenda muestra las unidades presentes y la tabla inferior enlaza cada código con la metadata normalizada."
        ),

        layer_source_ui(GEOLOGY_STEM),
        ...
        shiny::conditionalPanel(
          ...
          shiny::div(
            class = "geo-card",
            shiny::tags$h4("Unidades geológicas presentes"),
            shiny::div(
    ...
    shiny::moduleServer(
      ...
      function(
        ...
        status <- shiny::reactiveVal(
          "Delimita o carga una cuenca para consultar Geología."
        )
        ...
        shiny::observeEvent(
          current_key(),
          {
            result(NULL)
            status(
              "Cuenca disponible. Pulsa Cargar geología."
            )
          },
          ignoreInit = TRUE
        )
            ...
            tryCatch(
              {
                ...
                status(
                  "Localizando teselas geológicas..."
                )

                spatial <- read_geology_for_basin(
                  b
                )
                ...
                tab_info <- build_geology_table(
                  geology = spatial$geology,
                  basin = b
                )
        ...
        output$descargar_png <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            if (is.null(x)) {
              stop(
                "No hay un mapa geológico calculado para descargar."
              )
            }
        ...
        output$descargar_shp_recortado <- shiny::downloadHandler(
          filename = function() {
            paste0(
              "geologia_normalizado_recortado_",
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                geologia_recortado = clipped
              ),
              target_file = file,
              bundle_stem = "geologia_normalizado_recortado"
            )
          }
        )
        ...
        output$descargar_shp_mosaico <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                geologia_mosaico = mosaic
              ),
              target_file = file,
              bundle_stem = "geologia_normalizado_mosaico"
            )
          }
        )
