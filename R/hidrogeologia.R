...
# SUBMODULO: HIDROGEOLOGIA
# v1: criterio tematico congelado + fondo departamental + mapa A3 + metadata + exportacion | arquitectura independiente del orden de carga
# ============================================================

# Basado en el criterio temático congelado:
# fondo departamental limpio, sin hillshade, paleta categórica amplia.
hidrogeologia <- local({
  ...
  HYDROGEOLOGY_STEM <- "hidrogeologia_normalizada_disuelto"
  ...
  hydrogeology_colors <- function(codes) {
    codes <- unique(
      trimws(
        as.character(codes)
      )
  ...
  read_hydrogeology_metadata <- function() {
    if (exists(
      "hydrogeology",
  ...
  hydrogeology_tiled_dir <- function() {
    file.path(
      LAYERS_DIR,
      "medio_fisico",
      paste0(
  ...
  hydrogeology_original_file <- function() {
    file.path(
      LAYERS_DIR,
      "medio_fisico",
      paste0(
  ...
  hydrogeology_index <- function() {
    tiled_dir <- hydrogeology_tiled_dir()
    ...
    if (
      ...
      if (!"RELATIVE_PATH" %in% names(idx)) {
        stop(
          "El index.gpkg de Hidrogeología no contiene RELATIVE_PATH."
        )
      }
  ...
  read_hydrogeology_for_basin <- function(basin) {
    ...
    if (!is.null(idx)) {
      ...
      if (nrow(idx_hit) == 0L) {
        stop(
          "Ninguna tesela de Hidrogeología intersecta la cuenca activa."
        )
      }
      ...
      if (length(missing_tiles) > 0L) {
        stop(
          paste0(
            "Falta una tesela de Hidrogeología:\n",
            missing_tiles[1]
          )
        )
      }
      ...
      for (i in seq_along(selected_tiles)) {
        ...
        if (!"CODIGO" %in% names(g)) {
          stop(
            paste0(
              "La tesela de Hidrogeología no contiene CODIGO:\n",
              selected_tiles[i]
            )
          )
        }
      ...
      original <- hydrogeology_original_file()

      if (!file_nonempty_local(original)) {
        stop(
          paste0(
            "No se encontró la capa de Hidrogeología teselada ni el GPKG original.\n",
            "Esperado:\n",
            hydrogeology_tiled_dir(),
            "\nó\n",
            original
          )
        )
      ...
      if (length(layers) < 1L) {
        stop("El GPKG de Hidrogeología no contiene capas vectoriales.")
      }
      ...
      if (!"CODIGO" %in% names(g)) {
        stop("El GPKG de Hidrogeología no contiene CODIGO.")
      }
    ...
    if (is.na(crs_ref)) {
      stop("La capa de Hidrogeología no tiene CRS válido.")
    }
    ...
    hydrogeology_all <- do.call(
      rbind,
      pieces
    )

    hydrogeology_all <- sf::st_make_valid(
      hydrogeology_all
    )

    hydrogeology_all <- hydrogeology_all[
      !sf::st_is_empty(hydrogeology_all),
      ,
      drop = FALSE
    ]
    ...
    if (nrow(hydrogeology_all) == 0L) {
      stop("Hidrogeología no contiene unidades dentro de la cuenca.")
    }
    ...
    if (nrow(clipped) == 0L) {
      stop("La intersección hidrogeológica no produjo polígonos válidos.")
    }
  ...
  build_hydrogeology_table <- function(
      hydrogeology,
      basin
  ) {
    ...
    hydrogeology_utm <- sf::st_transform(
      hydrogeology,
      epsg
    )
    ...
    meta <- read_hydrogeology_metadata()
  ...
  draw_hydrogeology_map <- function(x) {
    basin <- sf::st_transform(
      x$basin,
      4326
    )

    hydrogeology <- sf::st_transform(
      x$hydrogeology,
      4326
    )
    ...
    cols <- hydrogeology_colors(codes)
    ...
    graphics::text(
      0.975,
      0.025,
      labels = layer_source_map_label(HYDROGEOLOGY_STEM),
      adj = c(1, 0),
      cex = 0.66,
      col = "grey30"
    )
  ...
  hydrogeology_datatable <- function(tab) {
    out <- tab
  ...
  ui <- function(id) {
    ...
    shiny::tagList(
      shiny::tags$style(
        shiny::HTML(
          paste0(
            ".geo-wrap{padding:14px 18px 28px 18px;max-width:1500px;margin:auto;}",
            ".geo-head{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:12px;}",
            ".geo-note{background:#f6f7f8;border-left:4px solid #607D8B;padding:10px 12px;margin:10px 0 14px 0;}",
            ".geo-card{border:1px solid #ddd;border-radius:7px;padding:12px 12px 18px 12px;background:white;margin-bottom:16px;}",
            ".geo-card h4{margin-top:0;}",
            ".geo-a3-frame{width:1188px;max-width:100%;margin:0 auto;aspect-ratio:420/297;}",
            ".geo-table-wrap{width:100%;overflow-x:auto;padding-bottom:14px;}",
            ".geo-table-wrap .dataTables_wrapper{width:100%;}",
            ".geo-table-wrap table.dataTable{width:100%!important;}"
          )
        )
      ),
      ...
      shiny::div(
        ...
        shiny::div(
          class = "geo-note",
          "La capa de hidrogeologia se carga únicamente para el área que intersecta la cuenca activa. La leyenda muestra las unidades presentes y la tabla inferior enlaza cada código con la metadata normalizada."
        ),

        layer_source_ui(HYDROGEOLOGY_STEM),
        ...
        shiny::conditionalPanel(
          ...
          shiny::div(
            class = "geo-card",
            shiny::tags$h4("Mapa hidrogeológico"),
            shiny::div(
              class = "geo-a3-frame",
              shiny::plotOutput(
                ns("mapa"),
          ...
          shiny::div(
            class = "geo-card",
            shiny::tags$h4("Unidades de suelo presentes"),
            shiny::div(
              class = "geo-table-wrap",
              DT::DTOutput(
                ns("tabla")
              )
            )
    ...
    shiny::moduleServer(
      ...
      function(
        ...
        status <- shiny::reactiveVal(
          "Delimita o carga una cuenca para consultar Hidrogeología."
        )
        ...
        shiny::observeEvent(
          current_key(),
          {
            result(NULL)
            status(
              "Cuenca disponible. Pulsa Cargar hidrogeología."
            )
          },
          ignoreInit = TRUE
        )
            ...
            tryCatch(
              {
                ...
                status(
                  "Cargando unidades de suelo..."
                )

                spatial <- read_hydrogeology_for_basin(
                  b
                )
                ...
                tab_info <- build_hydrogeology_table(
                  hydrogeology = spatial$hydrogeology,
                  basin = b
                )
        ...
        output$mapa <- shiny::renderPlot({
          x <- result()
          shiny::req(x)
          draw_hydrogeology_map(x)
        })

        output$tabla <- DT::renderDT({
          x <- result()
          shiny::req(x)
          hydrogeology_datatable(
            x$table
          )
        }, server = FALSE)
        ...
        output$descargar_png <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            if (is.null(x)) {
              stop(
                "No hay un mapa de hidrogeologia calculado para descargar."
              )
            }
        ...
        output$descargar_shp_recortado <- shiny::downloadHandler(
          filename = function() {
            paste0(
              "hidrogeologia_normalizado_recortado_",
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                hidrogeologia_recortado = clipped
              ),
              target_file = file,
              bundle_stem = "hidrogeologia_normalizado_recortado"
            )
          }
        )
        ...
        output$descargar_shp_mosaico <- shiny::downloadHandler(
          filename = function() {
            paste0(
              "hidrogeologia_normalizado_mosaico_",
              format(Sys.Date(), "%Y%m%d"),
              ".zip"
            )
          },
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                hidrogeologia_mosaico = mosaic
              ),
              target_file = file,
              bundle_stem = "hidrogeologia_normalizado_mosaico"
            )
          }
        )
