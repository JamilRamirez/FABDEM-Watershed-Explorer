...
# Basado en el criterio congelado de Geología:
...
geomorfologia <- local({
  ...
  GEOMORPHOLOGY_STEM <- "geomorfologia_normalizada_disuelto"
  ...
  geomorphology_colors <- function(codes) {
    codes <- unique(
      trimws(
        as.character(codes)
      )
  ...
  read_geomorphology_metadata <- function() {
    if (exists(
      "geomorphology",
  ...
  geomorphology_tiled_dir <- function() {
    file.path(
      LAYERS_DIR,
      "medio_fisico",
      paste0(
  ...
  geomorphology_original_file <- function() {
    file.path(
      LAYERS_DIR,
      "medio_fisico",
      paste0(
...
read_geomorphology_for_basin <- function(basin) {
    ...
    if (!is.null(idx)) {
      ...
      if (nrow(idx_hit) == 0L) {
        stop(
          "Ninguna tesela de Geomorfología intersecta la cuenca activa."
        )
      }
      ...
      if (length(missing_tiles) > 0L) {
        stop(
          paste0(
            "Falta una tesela de Geomorfología:\n",
            missing_tiles[1]
          )
        )
      }
      ...
      original <- geomorphology_original_file()

      if (!file_nonempty_local(original)) {
        stop(
          paste0(
            "No se encontró Geomorfología teselada ni el GPKG original.\n",
            "Esperado:\n",
            geomorphology_tiled_dir(),
            "\nó\n",
            original
      ...
      if (length(layers) < 1L) {
        stop("El GPKG de Geomorfología no contiene capas vectoriales.")
      }
      ...
      if (!"CODIGO" %in% names(g)) {
        stop("El GPKG de Geomorfología no contiene CODIGO.")
      }
    ...
    if (is.na(crs_ref)) {
      stop("La capa de Geomorfología no tiene CRS válido.")
    }
    ...
    geomorphology_all <- do.call(
      rbind,
      pieces
    )

    geomorphology_all <- sf::st_make_valid(
      geomorphology_all
    )

    geomorphology_all <- geomorphology_all[
      !sf::st_is_empty(geomorphology_all),
      ,
      drop = FALSE
    ]
    ...
    if (nrow(geomorphology_all) == 0L) {
      stop("Geomorfología no contiene unidades dentro de la cuenca.")
    }
    ...
    if (nrow(clipped) == 0L) {
      stop("La intersección geomorfológica no produjo polígonos válidos.")
    }
  ...
  build_geomorphology_table <- function(
      geomorphology,
      basin
  ) {
    ...
    geomorphology_utm <- sf::st_transform(
      geomorphology,
      epsg
    )
    ...
    meta <- read_geomorphology_metadata()
  ...
  draw_geomorphology_map <- function(x) {
    basin <- sf::st_transform(
      x$basin,
      4326
    )

    geomorphology <- sf::st_transform(
      x$geomorphology,
      4326
    )
    ...
    cols <- geomorphology_colors(codes)
    ...
    graphics::text(
      0.975,
      0.025,
      labels = layer_source_map_label(GEOMORPHOLOGY_STEM),
      adj = c(1, 0),
      cex = 0.66,
      col = "grey30"
    )
  ...
  geomorphology_datatable <- function(tab) {
    out <- tab
  ...
  ui <- function(id) {
    ...
    shiny::tagList(
      ...
      shiny::div(
        ...
        shiny::div(
          class = "geo-note",
          "La capa geomorfológica se carga únicamente para las teselas que intersectan la cuenca activa. La leyenda muestra las unidades presentes y la tabla inferior enlaza cada código con la metadata normalizada."
        ),

        layer_source_ui(GEOMORPHOLOGY_STEM),
        ...
        shiny::conditionalPanel(
          ...
          shiny::div(
            class = "geo-card",
            shiny::tags$h4("Mapa geomorfológico"),
            shiny::div(
              class = "geo-a3-frame",
              shiny::plotOutput(
                ns("mapa"),
          ...
          shiny::div(
            class = "geo-card",
            shiny::tags$h4("Unidades geomorfológicas presentes"),
            shiny::div(
    ...
    shiny::moduleServer(
      ...
      function(
        ...
        status <- shiny::reactiveVal(
          "Delimita o carga una cuenca para consultar Geomorfología."
        )
        ...
        shiny::observeEvent(
          current_key(),
          {
            result(NULL)
            status(
              "Cuenca disponible. Pulsa Cargar geomorfología."
            )
          },
          ignoreInit = TRUE
        )
            ...
            tryCatch(
              {
                ...
                status(
                  "Localizando teselas geomorfológicas..."
                )

                spatial <- read_geomorphology_for_basin(
                  b
                )
                ...
                tab_info <- build_geomorphology_table(
                  geomorphology = spatial$geomorphology,
                  basin = b
                )
        ...
        output$mapa <- shiny::renderPlot({
          x <- result()
          shiny::req(x)
          draw_geomorphology_map(x)
        })

        output$tabla <- DT::renderDT({
          x <- result()
          shiny::req(x)
          geomorphology_datatable(
            x$table
          )
        }, server = FALSE)
        ...
        output$descargar_png <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            if (is.null(x)) {
              stop(
                "No hay un mapa geomorfológico calculado para descargar."
              )
            }
        ...
        output$descargar_shp_recortado <- shiny::downloadHandler(
          filename = function() {
            paste0(
              "geomorfologia_normalizado_recortado_",
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                geomorfologia_recortado = clipped
              ),
              target_file = file,
        ...
        output$descargar_shp_mosaico <- shiny::downloadHandler(
          filename = function() {
            paste0(
              "geomorfologia_normalizado_mosaico_",
              format(Sys.Date(), "%Y%m%d"),
              ".zip"
            )
          },
