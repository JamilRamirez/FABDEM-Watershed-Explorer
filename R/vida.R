...
vida <- local({
  ...
  VIDA_STEM <- "vida_normalizada_disuelto"
  ...
  vida_index <- function() {
    ...
    if (
      ...
      if (!"RELATIVE_PATH" %in% names(idx)) {
        stop(
          "El index.gpkg de Zonas de vida no contiene RELATIVE_PATH."
        )
      }
  ...
  read_vida_for_basin <- function(basin) {
    ...
    if (!is.null(idx)) {
      ...
      if (nrow(idx_hit) == 0L) {
        stop(
          "Ninguna tesela de Zonas de vida intersecta la cuenca activa."
        )
      }
      ...
      if (length(missing_tiles) > 0L) {
        stop(
          paste0(
            "Falta una tesela de Zonas de vida:\n",
            missing_tiles[1]
          )
        )
      }
      ...
      original <- vida_original_file()

      if (!file_nonempty_local(original)) {
        stop(
          paste0(
            "No se encontró Zonas de vida teselada ni el GPKG original.\n",
            "Esperado:\n",
            vida_tiled_dir(),
            "\nó\n",
            original
      ...
      if (length(layers) < 1L) {
        stop("El GPKG de Zonas de vida no contiene capas vectoriales.")
      }
      ...
      if (!"CODIGO" %in% names(g)) {
        stop("El GPKG de Zonas de vida no contiene CODIGO.")
      }
    ...
    if (is.na(crs_ref)) {
      stop("La capa de Zonas de vida no tiene CRS válido.")
    }
    ...
    vida_all <- sf::st_make_valid(
      vida_all
    )

    vida_all <- vida_all[
      !sf::st_is_empty(vida_all),
      ,
      drop = FALSE
    ]
    ...
    if (nrow(vida_all) == 0L) {
      stop("Zonas de vida no contiene unidades dentro de la cuenca.")
    }
    ...
    if (nrow(clipped) == 0L) {
      stop("La intersección zonas de vida no produjo polígonos válidos.")
    }
  ...
  draw_vida_map <- function(x) {
    basin <- sf::st_transform(
      x$basin,
      4326
    )

    vida <- sf::st_transform(
      x$vida,
      4326
    )
    ...
    graphics::text(
      0.975,
      0.025,
      labels = layer_source_map_label(VIDA_STEM),
      adj = c(1, 0),
      cex = 0.66,
      col = "grey30"
    )
    ...
    shiny::tagList(
      shiny::tags$style(
        shiny::HTML(
          paste0(
            ...
            ".vid-table-wrap{width:100%;overflow-x:auto;padding-bottom:14px;}",
            ".vid-table-wrap .dataTables_wrapper{width:100%;}",
            ".vid-table-wrap table.dataTable{width:100%!important;}"
          )
        )
      ),
      ...
      shiny::div(
        ...
        shiny::conditionalPanel(
          ...
          shiny::div(
            class = "vid-card",
            shiny::tags$h4("Mapa de zonas de vida"),
            shiny::div(
              class = "vid-a3-frame",
              shiny::plotOutput(
                ns("mapa"),
          ...
          shiny::div(
            class = "vid-card",
            shiny::tags$h4("Zonas de vida presentes"),
            shiny::div(
              class = "vid-table-wrap",
              DT::DTOutput(
                ns("tabla")
              )
            )
    ...
    shiny::moduleServer(
      ...
      function(
        ...
        status <- shiny::reactiveVal(
          "Delimita o carga una cuenca para consultar Zonas de vida."
        )
        ...
        shiny::observeEvent(
          current_key(),
          {
            result(NULL)
            status(
              "Cuenca disponible. Pulsa Cargar zonas de vida."
            )
          },
          ignoreInit = TRUE
        )
            ...
            tryCatch(
              {
                ...
                status(
                  "Localizando capa de zonas de vida..."
                )

                spatial <- read_vida_for_basin(
                  b
                )
        ...
        output$descargar_png <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            if (is.null(x)) {
              stop(
                "No hay un mapa de zonas de vida calculado para descargar."
              )
            }
        ...
        output$descargar_shp_recortado <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                vida_recortado = clipped
              ),
              target_file = file,
              bundle_stem = "vida_normalizado_recortado"
            )
          }
        )
        ...
        output$descargar_shp_mosaico <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              ...
              target_file = file,
              bundle_stem = "vida_normalizado_mosaico"
            )
          }
        )
