...
cobertura <- local({
  ...
  COBERTURA_STEM <- "cobertura_normalizada_disuelto"
  ...
  cobertura_colors <- function(codes) {
    codes <- unique(
      trimws(
        as.character(codes)
      )
  ...
  read_cobertura_metadata <- function() {
    if (exists(
      "cobertura",
  ...
  cobertura_original_file <- function() {
    file.path(
      LAYERS_DIR,
      "clima_superficie",
      paste0(
  ...
  cobertura_index <- function() {
    ...
    if (
      ...
      if (!"RELATIVE_PATH" %in% names(idx)) {
        stop(
          "El index.gpkg de Cobertura y uso no contiene RELATIVE_PATH."
        )
      }
  ...
  read_cobertura_for_basin <- function(basin) {
    ...
    if (!is.null(idx)) {
      ...
      if (nrow(idx_hit) == 0L) {
        stop(
          "Ninguna tesela de Cobertura y uso intersecta la cuenca activa."
        )
      }
      ...
      if (length(missing_tiles) > 0L) {
        stop(
          paste0(
            "Falta una tesela de Cobertura y uso:\n",
            missing_tiles[1]
          )
        )
      }
      ...
      original <- cobertura_original_file()

      if (!file_nonempty_local(original)) {
        stop(
          paste0(
            "No se encontró Cobertura y uso teselada ni el GPKG original.\n",
            "Esperado:\n",
            cobertura_tiled_dir(),
            "\nó\n",
            original
      ...
      if (length(layers) < 1L) {
        stop("El GPKG de Cobertura y uso no contiene capas vectoriales.")
      }
      ...
      if (!"CODIGO" %in% names(g)) {
        stop("El GPKG de Cobertura y uso no contiene CODIGO.")
      }
    ...
    if (is.na(crs_ref)) {
      stop("La capa de Cobertura y uso no tiene CRS válido.")
    }
    ...
    cobertura_all <- do.call(
      rbind,
      pieces
    )

    cobertura_all <- sf::st_make_valid(
      cobertura_all
    )

    cobertura_all <- cobertura_all[
      !sf::st_is_empty(cobertura_all),
      ,
      drop = FALSE
    ]
    ...
    if (nrow(cobertura_all) == 0L) {
      stop("Cobertura y uso no contiene unidades dentro de la cuenca.")
    }
    ...
    if (nrow(clipped) == 0L) {
      stop("La intersección cobertura y uso no produjo polígonos válidos.")
    }
  ...
  build_cobertura_table <- function(
      cobertura,
      basin
  ) {
  ...
  draw_cobertura_map <- function(x) {
    basin <- sf::st_transform(
      x$basin,
      4326
    )

    cobertura <- sf::st_transform(
      x$cobertura,
      4326
    )
    ...
    graphics::text(
      0.975,
      0.025,
      labels = layer_source_map_label(COBERTURA_STEM),
      adj = c(1, 0),
      cex = 0.66,
      col = "grey30"
    )
  ...
  cobertura_datatable <- function(tab) {
    out <- tab
  ...
  ui <- function(id) {
    ...
    shiny::tagList(
      shiny::tags$style(
        shiny::HTML(
          paste0(
            ".cob-wrap{padding:14px 18px 28px 18px;max-width:1500px;margin:auto;}",
            ".cob-head{display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:12px;}",
            ".cob-note{background:#f6f7f8;border-left:4px solid #607D8B;padding:10px 12px;margin:10px 0 14px 0;}",
            ".cob-card{border:1px solid #ddd;border-radius:7px;padding:12px 12px 18px 12px;background:white;margin-bottom:16px;}",
            ".cob-card h4{margin-top:0;}",
            ".cob-a3-frame{width:1188px;max-width:100%;margin:0 auto;aspect-ratio:420/297;}",
            ".cob-table-wrap{width:100%;overflow-x:auto;padding-bottom:14px;}",
            ".cob-table-wrap .dataTables_wrapper{width:100%;}",
            ".cob-table-wrap table.dataTable{width:100%!important;}"
          )
        )
      ),
      ...
      shiny::div(
        ...
        shiny::div(
          class = "cob-note",
          "La capa de cobertura y uso se carga únicamente para las geometrías que intersectan la cuenca activa. La leyenda muestra las clases presentes y la tabla inferior enlaza cada código con la metadata normalizada."
        ),

        layer_source_ui(COBERTURA_STEM),
        ...
        shiny::conditionalPanel(
          ...
          shiny::div(
            class = "cob-card",
            shiny::tags$h4("Mapa de cobertura y uso"),
            shiny::div(
              class = "cob-a3-frame",
              shiny::plotOutput(
                ns("mapa"),
          ...
          shiny::div(
            class = "cob-card",
            shiny::tags$h4("Clases de cobertura y uso presentes"),
            shiny::div(
              class = "cob-table-wrap",
              DT::DTOutput(
                ns("tabla")
              )
            )
    ...
    shiny::moduleServer(
      ...
      function(
        ...
        status <- shiny::reactiveVal(
          "Delimita o carga una cuenca para consultar Cobertura y uso."
        )
        ...
        shiny::observeEvent(
          current_key(),
          {
            result(NULL)
            status(
              "Cuenca disponible. Pulsa Cargar cobertura y uso."
            )
          },
          ignoreInit = TRUE
        )
            ...
            tryCatch(
              {
                ...
                status(
                  "Localizando capa de cobertura y uso..."
                )

                spatial <- read_cobertura_for_basin(
                  b
                )
                ...
                tab_info <- build_cobertura_table(
                  cobertura = spatial$cobertura,
                  basin = b
                )
        ...
        output$descargar_png <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            if (is.null(x)) {
              stop(
                "No hay un mapa de cobertura y uso calculado para descargar."
              )
            }
        ...
        output$descargar_shp_recortado <- shiny::downloadHandler(
          filename = function() {
            paste0(
              "cobertura_normalizado_recortado_",
          ...
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                cobertura_recortado = clipped
              ),
              target_file = file,
              bundle_stem = "cobertura_normalizado_recortado"
            )
          }
        )
        ...
        output$descargar_shp_mosaico <- shiny::downloadHandler(
          filename = function() {
            paste0(
              "cobertura_normalizado_mosaico_",
              format(Sys.Date(), "%Y%m%d"),
              ".zip"
            )
          },
          content = function(file) {
            ...
            write_shapefile_zip_bundle(
              layers = list(
                cobertura_mosaico = mosaic
              ),
              target_file = file,
              bundle_stem = "cobertura_normalizado_mosaico"
            )
          }
        )
