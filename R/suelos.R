...
suelos <- local({
  ...
  SOILS_STEM <- "suelos_normalizada_disuelto"
  ...
  soils_index <- function() {
    tiled_dir <- soils_tiled_dir()
    ...
    if (
      ...
      if (!"RELATIVE_PATH" %in% names(idx)) {
        stop(
          "El index.gpkg de Suelos no contiene RELATIVE_PATH."
        )
      }
  ...
  read_soils_for_basin <- function(basin) {
    ...
    if (!is.null(idx)) {
      ...
      if (nrow(idx_hit) == 0L) {
        stop(
          "Ninguna tesela de Suelos intersecta la cuenca activa."
        )
      }
      ...
      for (i in seq_along(selected_tiles)) {
        ...
        if (!"CODIGO" %in% names(g)) {
          stop(
            paste0(
              "La tesela de Suelos no contiene CODIGO:\n",
              selected_tiles[i]
            )
          )
        }
      ...
      original <- soils_original_file()

      if (!file_nonempty_local(original)) {
        stop(
          paste0(
            "No se encontró la capa de Suelos teselada ni el GPKG original.\n",
            "Esperado:\n",
            soils_tiled_dir(),
            "\nó\n",
            original
      ...
      if (length(layers) < 1L) {
        stop("El GPKG de Suelos no contiene capas vectoriales.")
      }
      ...
      if (!"CODIGO" %in% names(g)) {
        stop("El GPKG de Suelos no contiene CODIGO.")
      }
    ...
    if (is.na(crs_ref)) {
      stop("La capa de Suelos no tiene CRS válido.")
    }
    ...
    soils_all <- sf::st_make_valid(
      soils_all
    )

    soils_all <- soils_all[
      !sf::st_is_empty(soils_all),
      ,
      drop = FALSE
    ]
    ...
    if (nrow(soils_all) == 0L) {
      stop("Suelos no contiene unidades dentro de la cuenca.")
    }
    ...
    if (nrow(clipped) == 0L) {
      stop("La intersección de suelos no produjo polígonos válidos.")
    }
  ...
  draw_soils_map <- function(x) {
    basin <- sf::st_transform(
      x$basin,
      4326
    )

    soils <- sf::st_transform(
      x$soils,
      4326
    )
    ...
    graphics::text(
      0.975,
      0.025,
      labels = layer_source_map_label(SOILS_STEM),
      adj = c(1, 0),
      cex = 0.66,
      col = "grey30"
    )
  ...
  ui <- function(id) {
    ...
    shiny::tagList(
      ...
      shiny::div(
        ...
        shiny::div(
          class = "geo-note",
          "La capa de suelos se carga únicamente para el área que intersecta la cuenca activa. La leyenda muestra las unidades presentes y la tabla inferior enlaza cada código con la metadata normalizada."
        ),

        layer_source_ui(SOILS_STEM),
    ...
    shiny::moduleServer(
      ...
      function(
        ...
        status <- shiny::reactiveVal(
          "Delimita o carga una cuenca para consultar Suelos."
        )
        ...
        shiny::observeEvent(
          current_key(),
          {
            result(NULL)
            status(
              "Cuenca disponible. Pulsa Cargar suelos."
            )
          },
          ignoreInit = TRUE
        )
        ...
        output$descargar_png <- shiny::downloadHandler(
          ...
          content = function(file) {
            ...
            if (is.null(x)) {
              stop(
                "No hay un mapa de suelos calculado para descargar."
              )
            }
